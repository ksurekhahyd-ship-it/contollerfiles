...
#define RATIO_LMT_TXR9L 0x0000121E
#define RATIO_LMT_TXR9H 0x0000121F

#define RATIO_LMT_TXR10L 0x00001220
#define RATIO_LMT_TXR10H 0x00001221
...
unsigned long int TXR10_CNT,TXR9_CNT,TXR8_CNT,TXR7_CNT,TXR6_CNT,TXR5_CNT;

unsigned int TXR10_V,TXR9_V,TXR4_V,TXR3_V,TXR2_V,TXR1_V,CVT1_V,CVT2_V;
unsigned int TX10_V,TXR9_V,TXR8_V,TXR7_V,TXR6_V,TXR5_V,INV1_V,INV2_V;
...
unsigned int ratio_lmt_t8,ratio_lmt_t9,ratio_lmt_t10;
...
unsigned long int dc_cnt,dc_cnt1,dc_cnt2,dc_cnt3,cd_cnt;
...
unsigned long int Txr9_sum_cnt,Txr9_sum_cnt1,Txr9_sum_cnt2,Txr9_avg_cnt,Txr9_avg_cnt1;
unsigned long int Txr9_square_cnt,Txr9_sample_cnt1;
unsigned int Txr9_current,Txr9_curr_divider,Txr9_sample_cnt;
//////////////////////////////// txr10 current//////////////////////////////

unsigned long int Txr10_sum_cnt,Txr10_sum_cnt1,Txr10_sum_cnt2,Txr10_avg_cnt,Txr10_avg_cnt1;
unsigned long int Txr10_square_cnt,Txr10_sample_cnt1;
unsigned int Txr10_current,Txr10_curr_divider,Txr10_sample_cnt;
...
unsigned int Comm_Txr5_vlt,Comm_Txr6_vlt,Comm_Txr7_vlt,Comm_Txr8_vlt,Comm_Txr9_vlt,Comm_Txr10_vlt;
...
unsigned int Comm_Txr5_current,Comm_Txr6_current,Comm_Txr7_current,Comm_Txr8_current,Comm_Txr9_current,Comm_Txr10_current;
...
unsigned int ratio_lmt_t55,ratio_lmt_t66,ratio_lmt_t77,ratio_lmt_t88,ratio_lmt_t99,ratio_lmt_t100;
...
void ac_calc(void)
{
  ...
  /////////////////////AC Output Voltage Measurement Calc////////////////////	
	...
	//	Inv2_sum_cnt1=1000;
		...
		/////////////////////////// txr9////////////////////////////////
		Txr9_sum_cnt=Txr9_square_cnt;	
		Txr9_sum_cnt1=(unsigned long int)(Txr9_sum_cnt/m_sec_disp);
	//	Inv2_sum_cnt1=1000;
		Txr9_avg_cnt=(unsigned long int)(sqrt((double)Txr9_sum_cnt1));
		Txr9_avg_cnt1=(unsigned int)Txr9_avg_cnt;
		Txr9_current=(unsigned int)((Txr9_avg_cnt*90)/1360);  //1019		
		
		Comm_Txr9_current=Txr9_current;
	
		Txr9_square_cnt=0; 
		/////////////////////////// txr10////////////////////////////////
		Txr10_sum_cnt=Txr10_square_cnt;	
		Txr10_sum_cnt1=(unsigned long int)(Txr10_sum_cnt/m_sec_disp);
	//	Inv2_sum_cnt1=1000;
		Txr10_avg_cnt=(unsigned long int)(sqrt((double)Txr10_sum_cnt1));
		Txr10_avg_cnt1=(unsigned int)Txr10_avg_cnt;
		Txr10_current=(unsigned int)((Txr10_avg_cnt*90)/1360);  //1019		
		
		Comm_Txr10_current=Txr10_current;
...
void read_calib(void)
{
   ...
   ratio_lmt_t3=get_calibvalue(RATIO_LMT_TXR3H,RATIO_LMT_TXR3L);
	 ...
	 ratio_lmt_t9=get_calibvalue(RATIO_LMT_TXR9H,RATIO_LMT_TXR9L);
	 if(ratio_lmt_t9<700 || ratio_lmt_t9>3000)
	 ratio_lmt_t9=900;
	 
	 ratio_lmt_t99=ratio_lmt_t9;
	 
	 ratio_lmt_t10=get_calibvalue(RATIO_LMT_TXR10H,RATIO_LMT_TXR10L);
	 if(ratio_lmt_t9<700 || ratio_lmt_t9>3000)
	 ratio_lmt_t9=900;
	 
	 ratio_lmt_t100=ratio_lmt_t10;
...
disp_chk_txr9: 
    ...
    TXR9_V=(unsigned long int)((TXR8_CNT*110)/calib_lmt); 
  ...
  //cmd_lcd(0x88);
   ...
   if(key==0X05)  //if DOWN key press
     ...
     goto inc_shnt_txr9; //if SET key press
	 else if((key==0X03)&&(s_set1_flag==0))       
     goto s_set_txr9;
	 else 
	   {goto disp_chk_txr9;}
...
dec_shnt_txr9:
 ...
 s_set_txr9:
    temp2=temp1;
    c1=temp2;
	  data_write(RATIO_LMT_TXR9L,c1);
	  delay1(20);
	  c1=temp2>>8;
	  data_write(RATIO_LMT_TXR9H,c1);
	  delay1(20);
	  s_set1_flag=1;  //cnt1++;
	  while(key!=0x07);
    ...
    TXR10_V=(unsigned long int)((TXR10_CNT*110)/calib_lmt); 
  ...
  //cmd_lcd(0x88);
   ...
   if(key==0X05)  //if DOWN key press
     ...
     goto inc_shnt_txr10; //if SET key press
	 else if((key==0X03)&&(s_set1_flag==0))       
     goto s_set_txr10;
	 else 
	   {goto disp_chk_txr10;}
...
dec_shnt_txr10:
 ...
 s_set_txr10:
    temp2=temp1;
    c1=temp2;
	  data_write(RATIO_LMT_TXR10L,c1);
	  delay1(20);
	  c1=temp2>>8;
	  data_write(RATIO_LMT_TXR10H,c1);
	  delay1(20);
	  s_set1_flag=1;  //cnt1++;
	  while(key!=0x07);
    ...
    { 
    	      ...
    	      switch(flag5)
						{
							case 0:         
										...
										{								        										
											  ...
											  dc_cnt+=adc_values[1];   //channel 0 600; 
												dc_cntr++;   
							...
							case 1:     
									  if(V1==1)
										{
												dc_cnt+=adc_values[2];   //channel 0 600; 
												dc_cntr++;   
                    ...
                    break;										
							case 2:
										if(V2==1)
										{
												dc_cnt+=adc_values[17];   //channel 0 600; 
												dc_cntr++;   
                    ...
                    break;										
							case 3:
										if(V3==1)
										{
												dc_cnt+=adc_values[18];   //channel 0 600; 
												dc_cntr++;   
                    ...
                    break;			*/		
							case 6:
									if(V8==1)
										{
											  dc_cnt+=adc_values[12];   //channel 0 600; 
												dc_cntr++;   
												
												if(dc_cntr>=200)
												{
													dc_cnt1=(unsigned long int)(dc_cnt/200);
													TXR10_CNT=(unsigned long int)dc_cnt1;
													
													
													if(TXR10_CNT<780)
													TXR10_V=(unsigned int)((dc_cnt1*100)/780);
										
													else													
													TXR10_V=(unsigned int)((dc_cnt1*110)/ratio_lmt_t100);
                    ...
                    break;					
							case 7:
										if(V8==1)
										{
											  dc_cnt+=adc_values[13];   //channel 0 600; 
												dc_cntr++;   
												
												if(dc_cntr>=200)
												{
													dc_cnt1=(unsigned long int)(dc_cnt/200);
													TXR9_CNT=(unsigned long int)dc_cnt1;
													
													
													if(TXR9_CNT<780)
													TXR9_V=(unsigned int)((dc_cnt1*100)/780);
										
													else													
													TXR9_V=(unsigned int)((dc_cnt1*110)/ratio_lmt_t99);
                    ...
                    break;				
							case 12:
										if(V8==1)
										{
											  dc_cnt+=adc_values[14];   //channel 0 600; 
												dc_cntr++;   
                    ...
                    break;					
							case 13:
										if(V13==1)
										{
												dc_cnt+=adc_values[15];   //channel 0 600; 
												dc_cntr++;   
                    ...
                    break;					
							case 14:
							      if(V14==1)
										{
												dc_cnt+=adc_values[16];   //channel 0 600; 
												dc_cntr++;   
		...
		//cd_cnt++;
			...
			/*		Cvt1_sample_cnt=adc_values[1]; 
					Cvt1_sample_cnt1=(unsigned int)cvt1_sample_cnt;
          ...
          //////////////////txr5//////////////////////
			    ...
			    Txr8_sample_cnt1=(unsigned long int)(2048-Txr8_sample_cnt);
				  ...
				  Txr8_sample_cnt1=(unsigned long int)(Txr8_sample_cnt-2048);				
					...
					Txr9_sample_cnt=adc_values[10];
					if(Txr9_sample_cnt<2048)
			    Txr9_sample_cnt1=(unsigned long int)(2048-Txr9_sample_cnt);
				  else if(Txr8_sample_cnt>=2048)
				  Txr8_sample_cnt1=(unsigned long int)(Txr9_sample_cnt-2048);				
					Txr8_square_cnt+=(Txr9_sample_cnt1*Txr9_sample_cnt1);
					
					Txr10_sample_cnt=adc_values[11];
					if(Txr10_sample_cnt<2048)
			    Txr10_sample_cnt1=(unsigned long int)(2048-Txr9_sample_cnt);
				  else if(Txr10_sample_cnt>=2048)
				  Txr8_sample_cnt1=(unsigned long int)(Txr10_sample_cnt-2048);				
					Txr8_square_cnt+=(Txr10_sample_cnt1*Txr10_sample_cnt1);
...
{ 
   ...
   //V0=0;V1=0;V2=0;V3=0;V4=0;V5=0;V6=0;V7=0;V8=0;V9=0;V10=0;V11=0;V12=0;V13=0;V14=0;V15=0; 
	 V0=1;V1=1;V2=1,V3=1;V4=1;V5=1;V6=1;V7=1;V8=1;V10=1;V11=1;V12=1;V13=1;V14=1;V15=1;
  // flag5=0;
	 disp_cntr1=5000;
	 ...
	 flag5=0; //MUL_A0=0; MUL_A1=0; MUL_A2=1; MUL_A3=0;   //Y4
    ...
    if(m_sec_disp>200)
		{
			...
			TXR3_V<=10 || TXR4_V<=10 || TXR5_V<=10 || TXR6_V<=10) // || TXR7_V<=10 || TXR8_V<=10||TXR9_V<=10||TXR10_V<=10)
			{
				  FAULT_PIN=0;
		  }		
		  else
